...
import { CatastrophicExternalError, ExternalError } from '@lib/external-exceptions';
import { delay } from '@testing/standalone-mode/standalone-functions';
import {
  defaultSetting,
  getSettingBoolean,
  getSettingNumber,
} from '@testing/standalone-mode/standalone-mode';
...
export class MemoryBreweriesData extends BreweriesData {
  ...
  getPage(page: number): Promise<Brewery[]> {
    return delay(() => {
      ...
      if (getSettingBoolean(getBreweriesFailedFirstTimeKey) && this.isFirstTime <= 1) {
        this.isFirstTime += 1;
        return Promise.reject(new ExternalError('get breweries'));
      }
      return this.breweries.slice((page - 1) * this.pageSize, page * this.pageSize);
